...
describe("Repertoire", () => {
  ...
  describe("getTrainingVariations", () => {
    ...
    it(`should not include variations that do not match the training mode`, () => {
      ...
      const movesFromPositions: Record<string, VariationMove[]> = {
        [positions[0]]: [expected[0][0]],
        [positions[1]]: [expected[0][1]],
        [positions[2]]: []
      };
      (training[positions[1]][moves[1]]
  ...
  it(`should end the variations after the move to train`, () => {
    const positions: string[] = ["fen0", "fen1", "fen2", "fen3", "fen4"];
    const moves: string[] = ["san0", "san1", "san2", "san3"];
    const descendants: Record<string, string[]> = {
    ...
    const expected = [_.take(variation[0], 2)];
    const sourceVariations: Record<string, Variation[]> = {
      [positions[0]]: [],
      [positions[1]]: [_.take(variation[0], 1)],
      [positions[2]]: [_.take(variation[0], 2)],
      [positions[3]]: [_.take(variation[0], 3)],
      [positions[4]]: [_.take(variation[0], 4)]
    };
    const movesFromPositions: Record<string, VariationMove[]> = {
      [positions[0]]: [variation[0][0]],
      [positions[1]]: [variation[0][1]],
      [positions[2]]: [variation[0][2]],
      [positions[3]]: [variation[0][3]],
      [positions[4]]: []
    };
    ...
    (repertoire.positions.descendantPositions as jest.Mock).mockImplementation(
      (fen: string) => descendants[fen]
    );
    (repertoire.positions.getSourceVariations as jest.Mock).mockImplementation(
      (fen: string) => sourceVariations[fen]
    );
    (repertoire.positions.movesFromPosition as jest.Mock).mockImplementation(
      (fen: string) => movesFromPositions[fen]
